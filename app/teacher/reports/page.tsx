...
export default function TeacherNoticesPage() {
  ...
  const send = (e: React.FormEvent) => {
    ...
    // Only allow sending notices to assigned class
    const targetClassId = teacherInfo.classId || classId;
    storeApi.addNotice(teacherInfo.name, title.trim(), message.trim(), targetClassId);
        ...
        <div className="flex gap-3 items-center justify-end">
          <Select value={filterClassId} onValueChange={setFilterClassId}>
            ...
            <SelectContent>
              ...
              {teacherInfo?.classId ? (
                <SelectItem value={teacherInfo.classId}>
            ...
            <form onSubmit={send} className="grid grid-cols-1 gap-3">
              {teacherInfo?.classId ? (
                <div className="px-3 py-2 border rounded-md bg-gray-50 text-gray-700">
                  Class: {state.classes.find(c => c.id === teacherInfo.classId)?.name || teacherInfo.classId}
                </div>
              ) : (
